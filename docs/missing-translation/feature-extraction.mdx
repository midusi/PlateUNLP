...
Cada valor $$x_i$$ se lo ubica sobre el eje X de la matriz de la imagen y se copian en una nueva imagen las columnas que corresponden al intervalo de columnas $$[x_i - S_\text{width}: x_i + S_\text{width}]$$ respecto a la matriz de la imagen original. $$S_\text{width}$$ es el ancho en pixeles de los segmentos a recortar, por defecto $$S_\text{width}=60$$. La subimagen correspondiente a cada $$x_i$$ la definimos como $$S_i$$

```math
\begin{equation}
S_i = \text{ImgSc}[...][ x_i - S_\text{width} : x_i + S_\text{width}]
\end{equation}
```
