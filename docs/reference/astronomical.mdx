...
* [Misc.](#misc)
  ...
  * [`getCosZenithDistance`](#getcoszenithdistance)
  * [`getAirmass`](#getairmass)
  * [`getAirmassFromCosZenithDistance`](#getairmassfromcoszenithdistance)
...
Computes the hour angle of a celestial object given its right ascension and the mean local sidereal time. It just subtracts the right ascension from the local sidereal time. Both have to be referred to the same equinox: the coordinates queried for the epoch of the observation, not the J2000 ones.
...
Computes the cosine of the zenith distance from the local hour angle, the declination and the observer's latitude. It is the vertical component of the horizontal unit vector that [`equatorialToHorizontal`](#equatorialtohorizontal) builds, so it equals the sine of the altitude angle, but it skips resolving the azimuth, which the airmass does not need.
...
Same magnitude as [`getAirmass`](#getairmass), taking the cosine of the zenith distance returned by [`getCosZenithDistance`](#getcoszenithdistance) instead of the altitude angle. Both forms are equivalent by the identity above; this one is the one used to compute the `AIRMASS` metadata.
