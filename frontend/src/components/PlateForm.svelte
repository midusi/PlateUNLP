<script>
  ...
  import {
    ...
    getScancolDef,
    getSoftwareDef,
  } from "../helpers/metadataUtilities";
  export let metadata, plateData;

  /* Transformación de datos al fonato:
  0:(3) ['OBSERVAT', 'PLATE-N', 'DIGITALI']
  1:(3) ['SCANNER', 'SCAN-RES', 'SCAN-COL']
  2:(3) ['SOFTWARE', undefined, undefined]
  */
  let arr = [];
    ...
    <div class="form-row">
      {#each fields as field}
        <div class="form-group col-md-4">
          {#if field !== undefined}
            <Field name={field} bind:value={plateData[field]} />
          {/if}
        </div>
      {/each}
    </div>
