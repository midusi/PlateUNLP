...
function useBoundingBoxesDrag(
  ...
  function startDragging(event: React.MouseEvent, box: BoundingBox) {
    ...
    if (image) {
      ...
      const offsetX =
        (event.clientX - rect.left - (box.x - zoomInfo.origin.x) * imageScale.x * zoomInfo.scale) /
        (imageScale.x * zoomInfo.scale)
      const offsetY =
        (event.clientY - rect.top - (box.y - zoomInfo.origin.y) * imageScale.y * zoomInfo.scale) /
  ...
  function handleDragging(event: React.MouseEvent) {
    if (draggedBB !== null) {
      ...
      if (image) {
        ...
        const mouseX =
          (event.clientX - rect.left) / (imageScale.x * zoomInfo.scale) + zoomInfo.origin.x
        const mouseY =
          (event.clientY - rect.top) / (imageScale.y * zoomInfo.scale) + zoomInfo.origin.y
        ...
        <select
          ...
          onChange={(e) => {
            ...
            if (class_info.name !== newSelected!.name) {
              setBoundingBoxes((prevBoxes) =>
                prevBoxes.map((b) => (b.id === box.id ? { ...b, class_info: newSelected! } : b)),
              )
...
function ImageBBDisplay({
  ...
  const { draggedBB, startDragging, handleDragging, stopDragging } = useBoundingBoxesDrag(
    imageRef,
    imageScale,
    zoomInfo,
    setBoundingBoxes,
  ...
  const { handleResizeStart, handleResizing, stopResizing } = useBoundingBoxesResizing(
    imageRef,
    imageScale,
    selectedBB,
    setBoundingBoxes,
