...
import type { SpectrumData } from "~/types/ProcessInfoForm"
  ...
  return (
    <>
      <div className="flex flex-col w-full" style={{ margin: "5% 10%" }}>
        <table className="min-w-full">
          ...
          <tbody>
            {spectrums.map((spectrum, index) => {
              const completedSteps = totalStepsCompleted(
                index,
                processInfo.processingStatus.specificSteps,
              )
              ...
              return (
                <tr key={spectrum.id} className="hover:bg-gray-50">
                  ...
                  <td className="border px-6 py-2">{`${completedSteps}/${stepsNum}`}</td>
                  <td className="border px-4 py-2">
                    {complete ? (
        ...
        <Button onClick={() => setSpecificObject(null)} className="bg-red-400 mt-20">
          Delete Selected
        </Button>
      </div>
    </>
