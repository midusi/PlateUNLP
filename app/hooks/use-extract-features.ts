...
export function useExtractFeatures(
  ...
  useEffect(() => {
    ...
    if (!scienceUrl || !lamp1Url || !lamp2Url) return
    // Obtener informacion de imagen SCIENCE
    obtainimageMatrix(scienceUrl, false)
      .then((science) => {
        if (!science.data || science.data.length === 0) return
        bag.scienceInfo = {
          data: science.data,
          width: science.width,
        ...
        obtainimageMatrix(lamp1Url, false)
          .then((lamp1) => {
            ...
            obtainimageMatrix(lamp2Url, false)
              .then((lamp2) => {
...
function extractLampData({
  ...
  countCheckpoints,
  segmentWidth,
}: extractLampDataProps): extractLampDataResponse {
