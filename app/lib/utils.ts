...
export function legendreAlgoritm(x: number[], y: number[], degree = -1): (value: number) => number {
  if (x.length !== y.length) {
    throw new CustomError(
      ErrorCodes.DIFFERENT_PROMP_SIZE,
      "Los arreglos de números recibidos deben tener el mismo tamaño.",
...
export async function downloadGrayscaleImage(input4d: tf.Tensor4D, nombreArchivo = "image.png") {
  // 1. Asegurate que es tensor 4D y grayscale
  if (input4d.rank !== 4 || input4d.shape[3] !== 1) {
    throw new Error("Se espera un tensor " + "[batch, height, width, 1] en escala de grises.")
  }

  // 2. Tomá la primera imagen del batch
  const single = input4d.slice([0, 0, 0, 0], [1, input4d.shape[1], input4d.shape[2], 1])
  const squeezed = single.squeeze([0, 3]) as tf.Tensor2D // shape [height, width]

  // 3. Convertí a Uint8Array para canvas (0–255)
  const data = await squeezed.toInt().data()
...
export async function downloadRGBAImage(input4d: tf.Tensor4D, nombreArchivo = "image.png") {
  // 1. Validación: Tensor 4D con 4 canales (RGBA)
  if (input4d.rank !== 4 || input4d.shape[3] !== 4) {
    throw new Error("Se espera un tensor de forma [1, height, width, 4] en formato RGBA.")
  }

  // 2. Sacar la imagen del batch y dejar shape [height, width, 4]
  const image = input4d.squeeze([0]) as tf.Tensor3D

  // 3. Convertir a Uint8
  const data = await image.mul(255).clipByValue(0, 255).toInt().data()
  const [h, w] = image.shape.slice(0, 2)

  // 4. Crear canvas y pintar imagen
  const canvas = document.createElement("canvas")
  canvas.width = w
  canvas.height = h
  const ctx = canvas.getContext("2d")!
  ...
  for (let i = 0; i < data.length; i++) {
    imageData.data[i] = data[i] // ya está en formato RGBA
  }
...
export async function fetchRawBuffer(url: string): Promise<[ArrayBuffer, number, number]> {
