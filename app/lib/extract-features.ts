...
/**
 * Which methods to use for extracting features in `extractScience` and `extractLamp`.
 */
type ExtractFeaturesOptions = {
	/**
	 * The amount of checkpoints used for the extraction. It behaves as `space-around`, where
	 * the distance between checkpoints is equal and the space between the first and last
	 * checkpoint and the edges of the image half the distance between checkpoints.
	 */
	checkpoints: number;
	/**
	 * The width of the segments used for the extraction in pixels.
	 */
	segmentWidth: number;
	/**
	 * The function to use for fitting the data.
	 * - `"linal-regression"`: Uses linear regression to fit the data.
	 * - `"spline"`: Uses a cuadratic spline to fit the data.
	 */
...
>({
	...
	segmentWidth,
	fitFunction,
}: extractScienceProps<T>) {
	/** Convertir el arreglo de pixeles recivido a tensor */
	const channels = 1; // Imagen en escala de grises
	const imgTensor = tf.tensor(science, [height, width, channels], "int32");

	/** Segmentar la imagen */
	/** Coordenadas X medias a lo largo de toda la imagen de SCIENCE */
	const sciencePoints = findXspacedPoints(width, countCheckpoints);
