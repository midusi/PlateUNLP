...
export function getJulianDate(date: string, ut: string) {
  ...
  const ΔT = math.divide((hour - 12) * 60 * 60 + minutes * 60 + seconds, 24 * 60 * 60)
...
export function getBesselianEpoch(jd: number) {
  ...
  const epoch = math.chain(jd).subtract(2415020.31352).divide(365.242198781).add(1900).done()
...
export async function getSiderealTime(
  ...
  const θ = normalizeAngle(
    math.bignumber(jd).sub(JD_J2000).mul("1.00273781191135448").add("0.779057273264").mul(math.tau),
  )
  ...
  const daysΔT = math.divide(math.bignumber(ΔT.value), 24 * 60 * 60) as BigNumber
  ...
  const r = math
    ...
    .multiply(rotationMatrix3D(math.bignumber(pm.value.y).neg().mul(ARCSEC_TO_RAD), "x"))
    .multiply(rotationMatrix3D(math.bignumber(pm.value.x).neg().mul(ARCSEC_TO_RAD), "y"))
