...
export function degToDMS(deg: number, opts: { digits?: number, sep?: string } = {}) {
  ...
  if (!Number.isSafeInteger(digits) || digits < 0) throw new RangeError("degToDMS: `digits` must be a positive number")
...
export function degToHMS(deg: number, opts: { digits?: number, sep?: string } = {}) {
  ...
  if (!Number.isSafeInteger(digits) || digits < 0) throw new RangeError("degToHMS: `digits` must be a positive number")
