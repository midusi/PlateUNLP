import spectrums from "@/generated/spectrums.json"
...
export type SpectrumPoint =
  (typeof spectrums.datasets)[number]["points"][number]

export function getMaterialSpectralData(
  material: LampMaterial,
): SpectrumPoint[] {
  const dataset = spectrums.datasets.find((i) => i.id === "nist")!
  const materials = new Set(
    material.split("-").flatMap((m) => [m, `${m} I`, `${m} II`]),
  )
