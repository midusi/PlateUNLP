...
export const updateProject = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		...
		const sanitizedViewers = viewers.filter(
			(userId) => !editors.includes(userId),
		);
		...
		await db.transaction(async (tx) => {
			...
			await tx
				...
				.where(
					and(
						eq(userToProject.projectId, projectId),
						ne(userToProject.userId, userId),
					),
				);
...
export const updateProjectQueryOptions = (
	...
	queryOptions({
		...
		queryFn: () =>
			updateProject({ data: { userId, projectId, name, editors, viewers } }),
	});
