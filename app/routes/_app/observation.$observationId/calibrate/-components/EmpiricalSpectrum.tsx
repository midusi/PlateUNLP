...
export function EmpiricalSpectrum({
  ...
  const [lampPoints, setLampPoints, linesPalette, materialPoints, pixelToWavelengthFunction] =
    useGlobalStore((s) => [
      s.lampPoints,
      s.setLampPoints,
      s.linesPalette,
  ...
  function onClick(event: React.MouseEvent<SVGSVGElement>) {
    ...
    if (yMatch) {
      ...
      if (lampPoints.some((p) => p.x === newVal.x && p.y === newVal.y)) {
        setLampPoints(lampPoints.filter((point) => !(point.x === newVal.x && point.y === newVal.y)))
        return
      }
    ...
    <div ref={measureRef}>
      <svg width={width} height={height} onClick={interactable ? onClick : undefined}>
        ...
        <Group top={margin.top} left={margin.left}>
          ...
          />
          {showPixel && (
            <AxisTop scale={xScalePixel} label="Pixel" numTicks={Math.floor(xMax / 80)} />
          )}
          {preview && (
            <>
              <AxisBottom
