...
export function ReferenceLampSpectrum({
	...
	const data = useMemo(
		() =>
			getMaterialSpectralData(material as "He-Ne-Ar" | "Fe-Ne-Ar" | "Fe-Ne"),
		[material],
	);

	const { filteredData, xScale, yScale } = useMemo(() => {
		let min = 0;
		while (getX(data[min]) < minWavelength) min++;
		let max = data.length - 1;
		while (getX(data[max]) > maxWavelength) max--;
	...
	}, [data, minWavelength, maxWavelength]);
