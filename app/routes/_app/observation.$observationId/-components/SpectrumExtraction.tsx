...
type SpectrumExtractionOptions = {
	method: "spline" | "linear";
	countCheckpoints: number;
	segmentWidth: number;
};

export function SpectrumExtraction({
	title,
	image,
	options,
}: {
	...
	image: Uint8Array;
	options: SpectrumExtractionOptions;
}) {
	return (
		<div className="relative flex w-full items-center justify-center px-16">
			<div className="flex flex-col">
				{title && (
					<h2 className="flex justify-center pb-2 font-semibold text-slate-500 text-xl">
						{title}
					</h2>
				)}
				<ImageWithDraws
