...
export function ObservationMetadataForm({
	...
	const { mutate: computeMetadata, isPending: isComputingMetadata } =
		useMutation({
			mutationFn: async () => {
				try {
		...
		<Collapsible.Root defaultOpen={true}>
			<Card className="gap-0">
				<CardHeader className="flex items-center gap-4">
					...
					<form.Subscribe
						selector={(state) => getObservationMetadataCompletion(state.values)}
					>
				...
				>
					...
					<CardContent className="grid grid-cols-3 items-start gap-4">
						...
						<SelectFieldSimpleWithKnown
							...
							label="IMAGETYP"
							placeholder=""
							options={["object", "dark", "zero", "flat", "arc"].map(
								(v) => ({
									label: v,
									value: v,
								}),
